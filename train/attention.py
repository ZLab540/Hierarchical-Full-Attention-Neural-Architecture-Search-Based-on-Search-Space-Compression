...
class AttentionConv(nn.Module):
    ...
    def forward(self, x):
        ...
        k_out = k_out.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride) #沿H\W拆分出3*3的块
        v_out = v_out.unfold(2, self.kernel_size, self.stride).unfold(3, self.kernel_size, self.stride)

        k_out_h, k_out_w = k_out.split(self.out_channels // 2, dim=1)           #沿通道维度分为两个部分
